...
function to_page(pn,listArea,isNeedSetting) {
    ...
    $.ajax({
        ...
        error:function () {
            console.log("to_page--error--url="+global_url_map.blog_page_url);
...
function assign_global_var(pageInfo, listArea,isNeedSetting) {
    ...
    console.log('assign_global_var');
    console.log(pageInfo);
    console.log(pageInfo.size)
    erasure_preview_area(listArea);
...
function flush_preview_area(oldSize, newSize, listArea,isNeedSetting) {
    console.log("flush_preview_area  newSize=" + newSize + "  oldSize=" + oldSize);
...
function erasure_preview_area(listArea) {
    ...
    for(let i=0;i<lenght;i++){
        ...
        // $("#itemArea" + i).addClass('fadeOutRightBig');
        // $("#itemArea" + i).remove();
    }
    listArea.empty();
}
...
function blog_info_modal_reset(item,modal) {
    console.log('blog_info_modal_init');
...
function listItemDelete(delete_url) {
    ...
    $.ajax({
        ...
        type: "DELETE",
        success: function (result) {
            console.log("itemDelete--ajax--result:" + result);
            if (result.code == 100) {
                toastr.success("删除成功!");
                to_page(globalCurrentPage,global_listArea,global_isNeedSetting);
            } else {
