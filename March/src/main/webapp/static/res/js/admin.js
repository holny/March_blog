...
function init_admin_page(type,pn,listArea,searchForm,listSortArea,itemInfoModal,switchTab){
    ...
    switchTab.on('shown.bs.tab', function (e) {
        ...
        console.log("tab show event---activeTab:"+activeTab+"  prevTab:"+previousTab);
        if(activeTab=="Draft"){
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if(result.code==100) {
                ...
                console.log(listSearchMap);
                console.log("获取page失败,");
                console.log(result);
            }
        },
        error:function () {
            console.log("to_page--error--url="+admin_url_map.admin_global_search_url);
        }
    });
...
function init_list_search_form(type,searchForm) {
    console.log('init_list_search_form--type=');
    console.log(type);
    if(type=='blog'||type=='draft'){
...
function reset_search(type,searchForm) {
    console.log("reset_search--type="+type+' --admin_global_search_map='+admin_global_search_map);
    if(type=='blog'||type=='draft') {
...
function start_search(type,searchForm) {
    console.log("start_search--type="+type);
...
function init_user_search_info(searchMap,type,searchForm) {
    console.log('init_user_search_info');
...
function init_blog_search_info(searchMap,type,searchForm) {
    console.log('init_blog_search_info');
    let searchAnSortInfo = searchMap.sourceFrom;
    if(checkIsNullOrEmpty(searchMap.userName)){
        searchAnSortInfo+='>>userName';
    }else{
...
function add_list_item(index, item,type,tbodyArea) {
    console.log('add_list_item--index='+index+'  --type='+type+'  --item='+item);
    if(type=='blog') {
        let trArea = $('<tr></tr>')
...
function item_setting(index,type,item) {
    console.log('item_setting--index='+index+'  --type='+type);
    console.log(item);
    if(type=='blog'||type=='draft'){
        ...
        $('.blogModalConfirmBtn'+index).click(function () {
            console.log('confirmSave--index='+index);
            let blogMap = {
...
function user_edit_save_with_check(userMap,modal,type) {
    ...
    console.log("user_edit_save_with_check");
    console.log(userMap);
...
function admin_save_user_info(map,type,url) {
    // let url;
    // if(type=='blog'||type=='draft'){
    //     url = global_url_map.context_base+admin_url_map.admin_blog_edit_url;
    // }
    console.log(map);
    $.ajax({
        url: url,
        ...
        success: function (result) {
            ...
            if (result.code == 100) {
                ...
                if(admin_global_listArea){
                    console.log("刷新页面");
                    console.log(admin_global_current_page);
...
function item_delete(index,type,item) {
    console.log('item_delete--index='+index+'  --type='+type);
    console.log(item);
    if(type=='blog'||type=='draft'){
        admin_global_modal.children().remove();
        let modal_content = $('<div></div>').addClass('modal-content');
        ...
        admin_global_modal.on('click','.modalBlogDeleteConfirmBtn'+index,function () {
            console.log('modalBlogDeleteConfirmBtn--index='+index);
        ...
        admin_global_modal.on('click','.modalUserDeleteConfirmBtn'+index,function () {
            console.log('modalUserDeleteConfirmBtn--index='+index);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.code == 100) {
                ...
                console.log("itemDelete--ajax--删除失败");
            }
