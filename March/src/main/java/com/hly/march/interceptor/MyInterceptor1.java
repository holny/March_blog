package com.hly.march.interceptor;

import com.hly.march.controller.MainController;
import com.hly.march.controller.UserController;
import com.hly.march.entity.User;
import com.hly.march.exception.MyForbiddenException;
import com.hly.march.schedule.ProjectSchedule;
...
public class MyInterceptor1 extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        log.info("MyInterceptor1---preHandle执行了");
        Subject subject = SecurityUtils.getSubject();
        // 这里是作用在controller之前判断用户是否被封禁，从而禁止登录
        if (subject.isRemembered() || subject.isAuthenticated()) {
            User comer = (User)subject.getPrincipal();
            log.info("MyInterceptor1---preHandle    comer:{}",comer.toString());
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler, ModelAndView modelAndView) throws Exception {
        ...
        log.info("postHandle执行了  comer host:{}",host);
