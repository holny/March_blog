...
import com.hly.march.utils.GetRequestInfo;
...
public class LoginAndRegisterController {
    ...
    public Msg login(HttpServletRequest request, @Validated({LoginAndRegisterController.Login.class}) User userAccount, BindingResult result) {
        if(result.hasErrors()){
            // 校验失败
...
//        String host2 = subject.getSession().getHost();
    ...
    public Msg register(HttpServletRequest request,@Validated({LoginAndRegisterController.Insert.class}) User register,BindingResult result) {
        if(result.hasErrors()){
            // 校验失败
            Map<String,Object> map = new HashMap<>();
            List<FieldError> errors =  result.getFieldErrors();
